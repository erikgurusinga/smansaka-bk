...
import { PageProps, CounselingSession, Student } from '@/types';
...
export default function GroupCounselingEdit({ session: sesi, students }: Props) {
    const [processing, setProcessing] = useState(false);
    const [selectedStudents, setSelectedStudents] = useState<Student[]>(sesi.students ?? []);
    ...
    const onSubmit = (data: FormData) => {
        setProcessing(true);
        router.put(route('counseling.group.update', sesi.id), data, {
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                ...
                { label: sesi.topic, href: route('counseling.group.show', sesi.id) },
                { label: 'Edit' },
            ]}
        >
            ...
            <div className="mx-auto max-w-2xl space-y-5">
                <div className="flex items-center gap-3">
                    <Link href={route('counseling.group.show', sesi.id)}>

...
import { PageProps, CaseConference, AcademicYear, PaginatedData } from '@/types';
...
interface Props extends PageProps {
    records: PaginatedData<CaseConference>;
...
export default function CaseConferencesIndex({
    records,
    academic_years,
    filters,
    permissions,
    ...
    const confirmDelete = () => {
        ...
        setProcessing(true);
        router.delete(route('case-conferences.destroy', deleteModal.item.id), {
            onSuccess: () => {
                setDeleteModal({ open: false, item: null });
                toast.success('Konferensi kasus dihapus.');
            },
            onError: handleError,
    ...
    const confirmBulkDelete = () => {
        setProcessing(true);
        router.delete(route('case-conferences.bulk-destroy'), {
        ...
        >
            ...
            <div className="space-y-5">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-xl font-semibold text-neutral-900">Konferensi Kasus</h1>
                        <p className="mt-0.5 text-sm text-neutral-500">
                            Rapat multi-pihak + notulen + tanda tangan
                        </p>
                    </div>
                    <div className="flex gap-2">
                        ...
                        {canWrite && (
                            <Link href={route('case-conferences.create')}>
                                <Button>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-neutral-100 bg-neutral-50 text-left text-xs font-medium tracking-wide text-neutral-500 uppercase">
                                    ...
                                    <th className="px-4 py-3">Terkait Kasus</th>
                            ...
                            <tbody className="divide-y divide-neutral-50">
                                {records.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={7}>
                                            <EmptyState description="Belum ada catatan konferensi kasus." />
                                        </td>
                                    </tr>
                                ) : (
                                    records.data.map((item) => (
                                        <tr key={item.id} className="hover:bg-neutral-50/50">
                                            ...
                                            <td className="px-4 py-3 text-sm text-neutral-600">
                                                {item.case_record ? (
                                                    <span className="truncate">
                                                        {item.case_record.title}
                                                        <span className="block text-xs text-neutral-400">
                                                            {item.case_record.student?.name ?? '—'}
                                                        </span>
                                                    </span>
                                                ) : (
                                            ...
                                            <td className="px-4 py-3 text-neutral-500">
                                                {item.participants?.length ?? 0} orang
                                            </td>
            ...
            <DeleteModal
                ...
                title="Hapus Konferensi Kasus"
                description={`Hapus konferensi "${deleteModal.item?.topic}"?`}

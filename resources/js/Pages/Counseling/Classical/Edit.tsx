...
import { PageProps, ClassicalGuidance, SchoolClass } from '@/types';
...
export default function ClassicalGuidanceEdit({
    record,
    classes,
    documentation,
    agreement,
    ...
    const onSubmit = (data: FormData) => {
        ...
        router.post(route('counseling.classical.update', record.id), fd, {
            forceFormData: true,
            onSuccess: () => toast.success('Bimbingan klasikal diperbarui.'),
            onError: (errs) => {
                handleError(errs);
                setProcessing(false);
            },
    ...
    return (
        <AuthenticatedLayout
            breadcrumbs={[
                ...
                { label: record.topic, href: route('counseling.classical.show', record.id) },
                { label: 'Edit' },
            ]}
        >
            <Head title={`Edit — ${record.topic}`} />

            <div className="mx-auto max-w-2xl space-y-5">
                <div className="flex items-center gap-3">
                    <Link href={route('counseling.classical.show', record.id)}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-100">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <Label htmlFor="cge-date">Tanggal</Label>
                                <Input id="cge-date" type="date" {...register('date')} />
                                <InputError message={errors.date?.message} />
                            </div>
                            <div className="space-y-1.5">
                                <Label htmlFor="cge-dur">Durasi (menit)</Label>
                                <Input
                                    id="cge-dur"
                                    type="number"
                                    min={1}
                            ...
                            <div className="col-span-2 space-y-1.5">
                                <Label htmlFor="cge-method">Metode</Label>
                                <Input id="cge-method" {...register('method')} />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="cge-topic">Topik / Tema</Label>
                            <Input id="cge-topic" {...register('topic')} />
                    ...
                    <div className="space-y-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-neutral-100">
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="cge-desc">Uraian / Materi</Label>
                            <Textarea id="cge-desc" rows={3} {...register('description')} />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="cge-eval">Evaluasi</Label>
                            <Textarea id="cge-eval" rows={2} {...register('evaluation')} />
                        </div>
                    </div>
